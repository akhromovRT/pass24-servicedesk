    ...
    <div class="help-content">
      ...
      <section id="sla" class="help-section">
        ...
        <ul>
          ...
          <li><b>Когда таймер «Решение» на паузе</b>:
            <ul>
              <li>Ваше публичное сообщение — последнее в переписке (ждём ответ клиента). Пауза ставится автоматически, менять статус вручную не нужно.</li>
              <li>Статус заявки = <code>WAITING_FOR_USER</code> («Ожидание ответа клиента»).</li>
              <li>Статус заявки = <code>ON_HOLD</code> («Отложена»).</li>
              <li>В любом из этих случаев под прогресс-баром «Решение» показывается серый бейдж <b>⏸ SLA на паузе</b>, а сам бар красится в серый.</li>
            </ul>
          </li>
          <li><b>Таймер «Первый ответ»</b> не паузится — он останавливается навсегда, как только вы напишете первый публичный комментарий (<code>first_response_at</code>).</li>
          <li><b>Пауза снимается</b>: когда клиент написал публичный ответ (веб / email / Telegram) или вы перевели тикет в активный статус (<code>in_progress</code>, <code>engineer_visit</code>). Накопленное время паузы прибавляется к эффективному дедлайну.</li>
          <li><b>Internal-комментарии</b> не влияют на SLA — клиент их не видит.</li>
          <li><b>Цвет шкалы SLA</b> на странице заявки (если не на паузе): зелёный → жёлтый → оранжевый → красный.</li>
        </ul>
      </section>
