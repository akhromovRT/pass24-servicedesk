...
- `agent_docs/guides/support-operations.md` — **регламент работы менеджера поддержки** (обработка заявок, email, база знаний). Раздел 3.7 — SLA-полоски в UI.
- `agent_docs/guides/sla.md` — **SLA-счётчики и полоски** (бизнес-часы, паузы, цвета, что важно менеджеру). См. ADR-005, ADR-017.
