...
### 2026-05-17 — report: управленческий отчёт за апрель + май и воспроизводимый SQL-скрипт метрик

**Контекст:** для ежемесячной отчётной презентации понадобилась сводка по тикетам за период 03.04–15.05.2026 — объём, сроки, SLA, темы обращений, нагрузка агентов, CSAT.

**Что сделано:**
- `backend/scripts/report_apr_may_2026.sql` — самодостаточный read-only SQL-отчёт из 15 секций (headline-метрики, воронка статусов, разрезы по приоритету/категории/продукту/типу/каналу/группе, топ-клиенты, нагрузка агентов, дневной объём, SLA по приоритетам, повторные обращения, CSAT-распределение, KB-контекст). Воспроизводимый — для следующих месяцев меняются только даты в CTE `period`.
- `docs/management-report-apr-may-2026.md` — готовый структурированный отчёт по данным прода (276 заявок), пригодный для вставки в презентацию.

**Найденные проблемы данных (вынесены в рекомендации отчёта):**
- Bus factor = 1: один агент обрабатывает 94% заявок и 95% решений.
- CSAT 3,0/5 при охвате сбора 8%, поляризованное распределение (7 оценок «1★»).
- 95% заявок не связаны с компанией-клиентом (`customer_id` пустой) — аналитика по клиентам невозможна.
- Дрейф справочника категорий: `Ticket.category` хранится как `Column(String)`, в проде накопились дубли (`app`/`app_issues`, `pass`/`passes`) и значения вне enum `TicketCategory`.
- `first_response_at` заполнен лишь у ~46% апрельских заявок — метрика первого ответа неполная (ставится только при переходе в `in_progress`).

**Баг в скрипте (исправлен):** колонка `open_at_end` считала открытый backlog как `resolved_at IS NULL` — ошибочно: `Ticket.transition()` (`backend/tickets/models.py`) ставит `resolved_at` только при переходе в RESOLVED, а прямое закрытие NEW→CLOSED оставляет поле пустым. Скрипт выдавал 85 «открытых» вместо реальных 21. Исправлено: колонка `cohort_still_open` считает по статусу (`status NOT IN ('resolved','closed')`). Сверено с `tickets_open_now` — совпало (21).

**Прод-нюансы (для будущих SQL-прогонов):** psql-роль на проде — `pass24` (не `postgres`), база — `pass24_servicedesk`; `priority`/`role` — нативные PG-enum с метками в ВЕРХНЕМ регистре; `users.id` — uuid, `tickets.assignee_id/creator_id` — varchar (нужен каст `::text`).

### Обновлено
- [x] Документация — новый отчёт `docs/management-report-apr-may-2026.md`, скрипт, `agent_docs/index.md`
- [ ] Тесты — не применимо (read-only аналитический SQL-скрипт)
- [ ] ADR — не требуется (архитектурного решения не принималось)

### Следующие шаги
- При желании оформить отчёт в слайды.
- Кандидаты на отдельные задачи: унификация справочника `category`, авто-привязка `customer_id` по домену e-mail, фиксация `first_response_at` по первому публичному комментарию агента.
