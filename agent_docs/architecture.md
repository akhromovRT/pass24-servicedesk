...
- RBAC: property_manager видит только свой проект, support_agent/admin — все. Шаблоны — read-only в MVP
...
- Telegram (@PASS24bot): webhook, двусторонняя переписка
...
└── 010_implementation_projects.py    # Проекты внедрения (7 таблиц + 2 поля в tickets)
```
...
- [ ] Deflection Rate dashboard (сколько тикетов закрыто статьёй из БЗ)
- [ ] Двусторонняя синхронизация компаний Bitrix24 ↔ ServiceDesk
- [ ] Integrations: Sigur, Trassir, ZKTeco
