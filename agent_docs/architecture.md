...
- **Embed-виджет для облачных сайтов клиентов** (ADR-014): loader-скрипт `chat-loader.js` + iframe на `/chat-widget`. Одна строка `<script>` на ~250 облачных сайтах PASS24. Те же API `/assistant/chat` и `/tickets/guest`, same-origin изнутри iframe (CORS не требуется). Инструкция: `docs/embed-ai-chat-guide.md`.
