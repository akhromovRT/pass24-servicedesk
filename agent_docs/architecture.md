...
- SLA pause: on_hold и waiting_for_user ставят таймер на паузу; engineer_visit — нет. Дополнительно — message-driven pause: SLA «Решение» автопаузится после публичного ответа агента, снимается ответом клиента (`recompute_sla_pause` через OR-семантику флагов `sla_paused_by_status`/`sla_paused_by_reply`)
...
- SLA-трекинг (first_response_at, resolved_at, working-hours pause). Единая точка истины — `compute_sla_state(ticket, now)` в `backend/tickets/sla_service.py` (используется и в API через `TicketRead.@model_validator`, и в watcher). Бизнес-часы: пн-пт 9-18 МСК (`backend/tickets/business_hours.py`). Пауза копит **бизнес-секунды** (только рабочее время), отрицательное `sla_remaining_seconds` = просрочено на abs(value). См. ADR-005, ADR-017
...
- Email (support@pass24online.ru): IMAP polling, ответы по тегу `[PASS24-xxxxxxxx]`; идемпотентность через `ticket_comments.email_message_id` (миграция 022) и `tickets.email_message_id` (миграция 026) + unique partial index на каждом поле — защищены оба inbound-пути (reply и new-ticket), ADR-010
...
- **Embed-виджет для облачных сайтов клиентов** (ADR-014): loader-скрипт `chat-loader.js` + iframe на `/chat-widget`. Одна строка `<script>` на ~250 облачных сайтах PASS24. Те же API `/assistant/chat` и `/tickets/guest`, same-origin изнутри iframe (CORS не требуется). Инструкция: `docs/embed-ai-chat-guide.md`. Конечный посетитель сайта может перетащить FAB-кнопку в любой из 4 углов через Pointer Events API (snap к ближайшему углу, выбор сохраняется в `localStorage['pass24-chat-corner']` per-domain) — drag только desktop, на mobile сохраняется bottom-sheet flow. Default `data-offset-y=88` (вместо 24) — чтобы по умолчанию проскакивать sticky-футеры с action-кнопками сайтов.
...
```
backend/
├── main.py                   # FastAPI app factory, lifespan (polling + SLA watcher), SPA-fallback middleware (ADR-018)
...
│   ├── business_hours.py     # Чистый модуль: пн-пт 9-18 МСК, _is_work_time, business_hours_between, deadline_with_business_minutes
│   ├── sla_service.py        # compute_sla_state(ticket, now) → SlaState — единая точка истины SLA для бэка и API
│   └── sla_watcher.py        # Фоновая проверка SLA каждые 5 мин (использует compute_sla_state)
...
└── scripts/
    ...
    ├── dedup_ticket_comments.py   # Чистка дублей комментариев до миграции 022 (ADR-010)
    └── dedup_tickets.py           # Merge дублей email-тикетов до миграции 026 (non-destructive, через merged_into_ticket_id)
...
- **SLA watcher**: каждые 5 минут — email-предупреждение за 30 мин (рабочих) до нарушения активной фазы (response пока нет первого ответа, иначе resolve). Тикеты на паузе пропускаются явно (`if state.is_paused: continue`)
