...
«Новый», куда стейкхолдеры пишут о мелких проблемах и желаниях по порталу
...
```
┌────────────────────┐      ┌─────────────────────┐      ┌───────────────┐
│  Bitrix24 «Новый»  │◄────►│   pass24-dev-agent  │◄────►│  Claude Code  │
│  (чат Messenger)   │      │  (asyncio, Python)  │      │   CLI + git   │
└────────────────────┘      └─────────────────────┘      └───────┬───────┘
       ▲                              │                          │
       │                              ▼                          ▼
...
После деплоя — в **тестовом** чате (не «Новый»):
...
1. Подтвердить **Bitrix chat ID** чата «Новый» — получить заранее через
   `im.recent.get` и вручную выбрать нужный.
